...
export async function collectSource(source){
 ...
 if(feedMatch){try{const feed=await parser.parseURL(abs(page.url,feedMatch[1]));return (feed.items||[]).slice(0,20).map(x=>({title:x.title||'',summary:clean(x.contentSnippet||x.content||''),publishedAt:x.isoDate||x.pubDate||null,url:x.link||'',source:source.name,category:source.category,sourcePriority:source.priority})).filter(x=>x.title&&x.url)}catch{}}
